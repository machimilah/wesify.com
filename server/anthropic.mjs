 ...
 *
 * Wesify makes several model calls per build — a turn per interview question, then an architecture pass,
 * then a research pass with web search — and the operator paying for them is whoever runs the
 * deployment. On a frontier model that adds up per company built, before Wesify has earned anything.
 *
 ...
 * schema means the model is never reached at all. Wesify shipped `maxItems` in its interview schema for
 * months without noticing, because every test stubs the model: the suites proved Wesify handled the
...
export function getClient() {
  if (!reasoningAvailable()) throw Object.assign(new Error('Wesify frontier research is not configured. Set ANTHROPIC_API_KEY to enable it.'), { status: 503 })
 ...
 * supports what, and having it go stale the week a new one ships, Wesify reads the complaint and drops
...
/** `fallback` names what Wesify does instead, so the operator is told which surface stopped and why. */
export function refusal(message, fallback = 'Wesify continued with its built-in reasoning.') {
...
export function conversationText(conversation) {
  return conversation
    ...
    .slice(-16)
    .map(item => `${item.role === 'assistant' ? 'Wesify' : 'Operator'}: ${item.content}`)
    .join('\n')
}
