 ...
 *
 * Imported for its side effect and imported first, because `db.mjs` decides whether Wesify has accounts
...
for (const name of ['.env.local', '.env']) {
  ...
  try {
    ...
    console.warn(`Wesify could not read ${name}: ${error?.message ?? error}`)
  }
}

// An empty value in a file means "not configured", not "configured as nothing". Without this, a
// blank ANTHROPIC_API_KEY line reads as a key and Wesify tries to run the interview on the server.
