/**
 * Where BO's API lives, which is not always where the interface lives.
 *
 ...
 * anyone can read, which is why BO's model keys are read only by the server and never named here.
 */
 ...
 *
 * Every client module goes through this rather than fetching a bare path, so pointing BO at a
...
/**
 * Cross-origin requests carry no cookies unless asked to, and BO's do have to.
 *
 * BO authenticates with a bearer token in a header rather than a cookie, so this changes nothing
