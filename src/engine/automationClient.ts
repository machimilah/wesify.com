...
export function loadAutomationWorkspace(workspaceId: string) {
  return request<AutomationWorkspace>(workspaceId, `/api/projects/${workspaceId}/automations`)
}

export function createAutomationConnector(workspaceId: string, input: { name: string; type: ConnectorType; endpointUrl: string }) {
  return request<AutomationConnector>(workspaceId, `/api/projects/${workspaceId}/connectors`, { method: 'POST', body: JSON.stringify(input) })
}
...
export function createManagedAutomation(workspaceId: string, input: { name: string; entityId: string; event: 'created' | 'updated'; connectorId: string }) {
  return request<ManagedAutomation>(workspaceId, `/api/projects/${workspaceId}/automations`, { method: 'POST', body: JSON.stringify(input) })
}

export function setManagedAutomationEnabled(workspaceId: string, automationId: string, enabled: boolean) {
  return request<ManagedAutomation>(workspaceId, `/api/projects/${workspaceId}/automations/${automationId}`, { method: 'PATCH', body: JSON.stringify({ enabled }) })
}

export function testManagedAutomation(workspaceId: string, automationId: string, sendLive = false) {
  return request<AutomationRun>(workspaceId, `/api/projects/${workspaceId}/automations/${automationId}/test`, { method: 'POST', body: JSON.stringify({ dryRun: !sendLive }) })
}
