...
export const researchDimensions: DimensionSpec[] = [
  {
    ...
    essential: true,
    options: [
      option('services', 'Services', 'Services', 'This company sells work performed for clients', 'BO is connecting clients, the work they buy, and the invoices that follow it.', /\b(agency|agencies|consultanc|consulting|advisory|professional services|law firm|accounting firm|architect|studio|freelance|marketing services|creative services|service business|we do the work)\w*/, ['crm.contacts', 'work.projects', 'finance.invoicing']),
      option('physical-products', 'Physical products', 'Physical products', 'This company moves physical goods', 'BO is connecting products, stock levels, and what leaves the building.', /\b(physical (product|goods)|sell goods|retail|shop|store|warehouse|distribut\w*|wholesal\w*|manufactur\w*|factory|produce goods|parts|merchandise)s?\b/, ['commerce.products', 'inventory.stock']),
      option('software', 'Software or digital products', 'Software or digital products', 'This company sells a digital product rather than physical delivery', 'BO is connecting accounts, subscriptions, and support instead of stock and shipping.', /\b(saas|software as a service|software platform|web app|mobile app|digital product|online course|digital only)s?\b/, ['commerce.products', 'support.tickets'], ['inventory.stock', 'logistics.shipping', 'manufacturing.production']),
      option('mixed', 'A mix', 'A mix of both', 'This company sells both work and things', 'BO is connecting the service side and the product side to the same client records.', undefined, ['crm.contacts', 'commerce.products']),
    ],
  },
  {
    id: 'revenue',
    ...
    essential: true,
    options: [
      option('recurring', 'Recurring', 'Monthly retainer or subscription', 'This company delivers recurring client work', 'BO is connecting clients, recurring agreements, renewals, and the invoices they generate.', /\b(retainers?|subscriptions?|recurring|monthly fees?|monthly plans?|memberships?|per month|annual (fee|plan)s?|renewals?)s?\b/, ['subscriptions.billing', 'finance.invoicing', 'crm.contacts']),
      option('per-project', 'Per project or milestone', 'Per project or milestone', 'Revenue is committed per piece of work rather than continuously', 'BO is connecting quotes, the work they become, and staged billing against it.', /\b(per project|fixed fee|project fee|milestone|progress billing|staged (payment|invoice)|per job|quote|estimate|bid)s?\b/, ['sales.quotes', 'work.projects', 'finance.invoicing']),
      option('on-completion', 'On completion', 'When the work is finished', 'Money arrives only after delivery, so unbilled finished work is a real risk', 'BO is connecting completed work directly to invoicing and outstanding balances.', /\b(on completion|after the (job|work)|once (the )?(work|job) is (done|finished)|invoice after|when we finish)s?\b/, ['finance.invoicing', 'finance.payments']),
      option('upfront', 'Upfront', 'Before the work starts', 'Cash is collected before delivery', 'BO is connecting payments and deposits ahead of the work they release.', /\b(upfront|up front|in advance|prepaid|pre-paid|deposit|pay before)s?\b/, ['finance.payments', 'finance.invoicing']),
      option('point-of-sale', 'At the point of sale', 'At checkout', 'Payment and delivery happen in the same moment', 'BO is connecting sales sessions, takings, and stock consumed at the counter.', /\b(checkout|point of sale|\bpos\b|at the till|cash register|in[- ]store purchase|table service|per cover)s?\b/, ['commerce.pos', 'finance.payments']),
    ],
  },
  {
    id: 'delivery',
    ...
    essential: true,
    options: [
      option('on-site', 'At the customer location', 'We travel to the customer', 'Work happens at customer locations, so scheduling and dispatch are operational, not administrative', 'BO is connecting work orders, the people assigned to them, and the assets they service.', /\b(on ?site|customer site|customer'?s? (home|premises|location)|visit|call ?out|field service|technician|engineer visits|install\w*|repair\w* at)s?\b/, ['work.scheduling', 'service.field-work', 'people.directory']),
      option('appointment', 'At our own location by appointment', 'Customers book time with us', 'Capacity is booked time, so the calendar is the operating system of the business', 'BO is connecting bookings, the people delivering them, and what each one bills.', /\b(appointment|booking|book a (slot|time)|reservation|clinic|salon|consultation slot|session|class)s?\b/, ['work.scheduling']),
      option('project', 'As a managed project', 'We run projects or campaigns', 'Work is delivered as managed engagements with their own scope and deadlines', 'BO is connecting projects, the tasks inside them, team capacity, and cost against budget.', /\b(project|campaign|engagement|deliverable|sprint|scope of work|brief)s?\b/, ['work.projects', 'work.tasks']),
      option('shipped', 'Shipped to the customer', 'We ship goods out', 'Fulfilment is physical, so the operation ends at a delivered shipment', 'BO is connecting orders, stock, and shipments with their delivery status.', /\b(ship\w*|courier|dispatch|freight|tracking number|delivery van|deliver goods|post it out)s?\b/, ['sales.orders', 'logistics.shipping', 'inventory.stock']),
      option('produced', 'Made in-house first', 'We make it ourselves', 'The company converts inputs into output, so production capacity governs delivery', 'BO is connecting production orders, materials, and quality checks before anything ships.', /\b(manufactur\w*|production line|assembl\w*|fabricat\w*|we (make|build|produce)|workshop|factory floor)s?\b/, ['manufacturing.production', 'inventory.stock']),
    ],
  },
  {
    id: 'customer',
    ...
    essential: false,
    options: [
      option('business', 'Other businesses', 'Other businesses', 'Customers are organizations, so accounts outlive individual contacts', 'BO is connecting company accounts, their contacts, and the agreements between them.', /\b(b2b|businesses|companies|corporate|enterprise|other firms|technology companies|smes?|agencies as clients)s?\b/, ['crm.contacts', 'sales.contracts']),
      option('consumer', 'Consumers', 'Consumers', 'Customers are individuals, so volume and repeat contact matter more than account hierarchy', 'BO is connecting individual customer records to their history and payments.', /\b(b2c|consumers?|general public|homeowners?|individuals|private clients|patients|guests|residents|students)s?\b/, ['crm.contacts']),
      option('mixed', 'Both', 'Both businesses and consumers', 'The company serves organizations and individuals through the same operation', 'BO is keeping one client record type that works for both.', undefined, ['crm.contacts']),
    ],
  },
  {
    id: 'workforce',
    ...
    essential: true,
    options: [
      option('solo', 'Solo operator', 'Just me', 'One person runs the operation, so people administration would be pure overhead', 'BO is leaving out staffing, leave, and payroll administration entirely.', /\b(solo|just me|only me|i work alone|one[- ]person|sole (trader|operator)|no employees|no team)s?\b/, [], ['people.time-off', 'people.attendance', 'people.payroll', 'people.recruiting', 'work.resources']),
      option('internal-team', 'Internal team', 'A small internal team', 'A fixed internal team carries the work, so assignment and capacity are the real constraint', 'BO is connecting the team directory to assigned work and workload.', /\b(internal team|our team|employees|staff|small team|team of \d+|\d+ (people|employees|staff))s?\b/, ['people.directory']),
      option('contractors', 'Employees and contractors', 'Employees and contractors', 'Delivery capacity is partly external, so who is engaged and on what terms must be tracked', 'BO is connecting internal people, contractors, and what each is committed to.', /\b(contractor|freelancer|subcontract\w*|outsourc\w*|external partner|associates)s?\b/, ['people.directory', 'procurement.suppliers']),
      option('shifts', 'Shift or crew based', 'Shifts or crews', 'Labour is scheduled in shifts, so the rota is an operating document', 'BO is connecting shifts, attendance, and the work each covers.', /\b(shifts?|rota|roster|crew|on call|night staff|opening hours cover)s?\b/, ['people.directory', 'people.attendance']),
    ],
  },
  {
    id: 'supply',
    ...
    essential: false,
    options: [
      option('materials', 'Materials or stock', 'Yes, materials or stock', 'Delivery depends on purchased inputs, so shortages stop the work', 'BO is connecting suppliers, purchase orders, and stock against the work that consumes it.', /\b(materials?|raw material|parts|components|ingredients?|supplier|vendor|purchase order|buy stock|source (goods|parts))s?\b/, ['procurement.suppliers', 'procurement.purchasing', 'inventory.stock']),
      option('subcontracted', 'Subcontracted work', 'Yes, subcontractors', 'Part of delivery is bought in, so external commitments carry cost and risk', 'BO is connecting subcontractors to the work and cost they are attached to.', /\b(subcontract\w*|outsourc\w*|third[- ]party (installer|team)|partner delivers)s?\b/, ['procurement.suppliers']),
      option('nothing', 'Nothing is purchased to deliver', 'No, nothing is purchased to deliver', 'Delivery consumes no purchased inputs, so procurement and stock would be dead weight', 'BO is leaving out purchasing, stock, and shipping.', /\b(no (materials|stock|inventory)|nothing physical|digital(ly)? only|services? only|we don'?t (buy|hold) (stock|materials))s?\b/, [], ['inventory.stock', 'inventory.warehouses', 'procurement.purchasing', 'logistics.shipping']),
    ],
  },
  {
    id: 'control',
    ...
    essential: false,
    options: [
      option('approvals', 'Internal approvals', 'Yes, things need approving internally', 'Work waits on decisions, so approvals are a queue that can be measured', 'BO is connecting approval requests to the records they hold up.', /\b(approv\w*|sign[- ]?off|authoris\w*|authoriz\w*|needs? my ok|manager (approves|signs))s?\b/, ['documents.approvals']),
      option('contracts', 'Signed agreements', 'Yes, signed agreements', 'Commitments are contractual, so terms, dates, and signatures are operational data', 'BO is connecting contracts, their dates, and the signatures that activate them.', /\b(contract|agreement|signature|e[- ]?sign|nda|terms are signed|msa)s?\b/, ['sales.contracts', 'documents.repository']),
      option('regulated', 'Regulatory obligations', 'Yes, regulatory requirements', 'The work carries external obligations, so evidence must be retained, not just produced', 'BO is connecting controls, their owners, and the evidence each one needs.', /\b(gdpr|hipaa|iso ?\d*|soc ?2|regulat\w*|licen[cs]\w*|inspection|statutory|audit requirement|certification)s?\b/, ['compliance.controls', 'documents.repository']),
      option('none', 'Nothing formal', 'No, nothing formal', 'Nothing external gates delivery, so governance surfaces would sit unused', 'BO is leaving approvals and compliance out of the first version.', undefined, []),
    ],
  },
]
