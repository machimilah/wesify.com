...
/*
 * LightPillar, from React Bits by David Haz.
 ...
 *
 * Typed for this codebase and otherwise left as it ships, so the next version upstream can be
 * compared against it line by line. The one behavioural change is `preserveDrawingBuffer`, which the
 * theme suite needs in order to read the canvas back and prove the hero is lit rather than black.
 */
...
export function LightPillar({
  ...
  useEffect(() => {
    ...
    try {
      renderer = new THREE.WebGLRenderer({
        ...
        stencil: false,
        depth: false,
        // Wesify's own addition: the theme suite reads pixels back out of this canvas to prove the
        // hero is actually lit rather than rendering black behind the headline.
  ...
  useEffect(() => {
    if (!materialRef.current) return
    const radians = (pillarRotation * Math.PI) / 180
    materialRef.current.uniforms.uPillarRotCos.value = Math.cos(radians)
    materialRef.current.uniforms.uPillarRotSin.value = Math.sin(radians)
  }, [pillarRotation])
