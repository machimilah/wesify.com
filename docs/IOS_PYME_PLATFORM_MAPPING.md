...
The source document is an input to Wesify, not a replacement specification. Wesify remains an AI-native platform that discovers one company, compiles a structured Business Model, and generates an isolated Command Center. Source ideas are accepted only when they strengthen that mechanism.
...
| Disposition | Meaning |
| --- | --- |
| `use-directly` | The principle fits Wesify and already has a valid platform implementation. |
| `generalize` | Keep the reusable principle, but make the data and behavior configurable. |
| `adapt` | Keep the idea only after adding Wesify controls, evidence, permissions, or versioning. |
...
| Source idea | Decision | Wesify primitive | Result |
| --- | --- | --- | --- |
...
| Diagnostic gaps | Use directly | Typed unknowns, confidence and exclusions | Wesify exposes missing knowledge instead of guessing. |
...
| Dominican payroll calculations | Jurisdiction module | External connector contract only | Wesify does not calculate payroll or labor obligations. |
...
| One fixed ERP schema | Do not use | Schema compiler with invariants | Wesify preserves company-specific construction. |
...
| External industry research | Adapt | Cited, expiring evidence | Stale research stops deciding what Wesify builds. |
...
4. Payroll, labor, tax and binding legal calculations stay outside Wesify core.
