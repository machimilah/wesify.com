...
try {
  ...
  // Ownership in the database, not a role header chosen by the browser, authorizes workspace actions.
  const specification = {
  ...
  /**
   * A second signed-in account gets nothing at all, and there is no way to give it anything.
   *
   * This used to be a role test: a colleague was added to the workspace as an employee, and the
   * question was whether they could read but not write. A workspace has one owner now and no second
   * member, so the question changed shape entirely — the answer is not "less than the owner", it is
   * "nothing", on every verb, with no header or role able to alter it.
   */
  const otherToken = tokenFor('user_member')
  await call('/api/auth/me', { token: otherToken })
  const otherRead = await call(`/api/projects/${workspaceId}/records/customers`, { token: otherToken, workspaceId })
  assert.equal(otherRead.status, 403, `another account read someone else's records (${otherRead.status})`)
  const otherWrite = await call(`/api/projects/${workspaceId}/records/customers`, { method: 'POST', token: otherToken, workspaceId, body: { name: 'Unauthorized' } })
  assert.equal(otherWrite.status, 403, 'another account wrote a record into someone else’s workspace')
  // And the workspace stays off their list, however many times they touch it.
  const otherAccount = await (await call('/api/auth/me', { token: otherToken })).json()
  assert.equal(otherAccount.workspaces.some(row => row.id === workspaceId), false, 'someone else’s workspace appeared in an account’s list')
